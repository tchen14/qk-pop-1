...
#pragma warning disable 0114
	...
	void FixedUpdate() {
		...
		PlayerInput(InputManager.input.MoveHorizontalAxis(),
					InputManager.input.isJumping(),
					InputManager.input.MoveVerticalAxis(),
					InputManager.input.isActionPressed());
	...
	private void PlayerInput(int xPlaneInput, bool yPlaneInput, int zPlaneInput, bool action) {
		if(action) {
			if(actionAvaiable && myActionState == actionState.prepState) { //Start Action
				...
				//actionComponent = null; //this was once uncommented /x\
			...
			NoActionLoop(xPlaneInput, (yPlaneInput ? 1 : 0), zPlaneInput);
		}
	}

	private void NoActionLoop(int xPlaneInput, int yPlaneInput, int zPlaneInput) {
	 ...
	 */
	private void UpdateMovement(int xPlaneMovement, int zPlaneMovement) {
	...
	private void UpdateJump(int yPlaneMovement) {
		if(grounded) {
			grounded = false;
