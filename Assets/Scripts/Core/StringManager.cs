using UnityEngine;
using System.Collections;

// This class houses all config strings
public static class StringManager {
	public const string BUILDNAME = "qk-pop";
	public const string FIRSTSCENE = "Menu";
	
	public const string TILESETDATA = "tilesetData";
	
	public const string SCENELIST = "sceneList";
	public const string SCENEPATH = "Assets/Scenes/";
	
	public const string ACHIEVEMENTLIST = "achievementList";

	public const string DEBUGKEYS = "/Scripts/_HelperScripts/debugKeys.json";
	public const string INPUTKEYS = "inputManager";
}
