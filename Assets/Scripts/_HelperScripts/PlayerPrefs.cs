...
namespace FFP
{
	...
	{
		private const string ENCRYPTIONKEY = "0u3)R#)UUzr8u3r89urUR#(*WUR#";
		private const string PREFIX = "qkpop_";
		...
		private static byte[] bytes = ASCIIEncoding.ASCII.GetBytes (ENCRYPTIONKEY + SystemInfo.deviceUniqueIdentifier.Substring (0, 4));
		...
		{
			return playerPrefsHashtable.ContainsKey (PREFIX + key);
		}
		
		public static void SetString (string key, string value)
		{
			if (!playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				playerPrefsHashtable.Add (PREFIX + key, value);
			} else {
				playerPrefsHashtable [PREFIX + key] = value;
			}
		...
		{			
			if (playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				return playerPrefsHashtable [PREFIX + key].ToString ();
			}
		...
		public static string GetString (string key, string defaultValue)
		{
			if (playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				...
				playerPrefsHashtable.Add (PREFIX + key, defaultValue);
				hashTableChanged = true;
				return defaultValue;
			}
		}
		...
		{			
			if (playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				return (int)playerPrefsHashtable [PREFIX + key];
			}
		...
		{			
			if (playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				return (long)playerPrefsHashtable [PREFIX + key];
			}
		...
		{			
			if (playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				return (float)playerPrefsHashtable [PREFIX + key];
			}
		...
		{			
			if (playerPrefsHashtable.ContainsKey (PREFIX + key)) {
				return (bool)playerPrefsHashtable [PREFIX + key];
			}
		...
		{
			playerPrefsHashtable.Remove (PREFIX + key);
		}
