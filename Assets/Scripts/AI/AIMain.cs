...
public class AIMain : MonoBehaviour
{
    ...
    private bool nextCheckpointRunning = false;
    ...
    private bool startnextCheckpointRunning;
    ...
    void Awake()
    {
        ...
        startnextCheckpointRunning = nextCheckpointRunning;
    ...
    void FixedUpdate()
    {
        ...
        if ((Vector3.Distance(transform.position, navPoint) < 3) && (chasing == false) && (nextCheckpointRunning == false))
        {
            nextCheckpointRunning = true;
            nextCheckpoint();
        }
        if (seesTarget == true)
        {
        ...
        if (PathwayCount <= Pathways.Count - 1)
        {
            ...
            switch (PathType[PathwayCount])
            {
                ...
                case 3:
                    if (LoopCount <= nofLoops[PathwayCount])
                    {
                        if (CheckpointCount < CheckpointScript.getPoints().Count && waiting == false)
                        {
                            Debug.Log("happens");
                            string CheckpointCountString = CheckpointCount.ToString();
                            ChangeNavPoint(CheckpointCountString, CheckpointScript.getPoints()[CheckpointCount]);
    ...
    public void Restart()
    {
        ...
        nextCheckpointRunning = startnextCheckpointRunning;
