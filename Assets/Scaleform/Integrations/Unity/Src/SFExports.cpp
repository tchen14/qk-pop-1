...
namespace GFxUnity3D
{
    ...
    public:
		...
		// ************Event Handling***************
		SF_UNITY_EXPORT bool SF_CALLING_CONV SF_HandleMouseEvent(SInt64 movieId, float x, float y, int icase, int buttonType, float scrollDelta)
		{
			Lock::Locker locker(&SFUnityLock);
			if(movieId == 0 || !CheckForNullManager(pManager)) return false;
			return pManager->HandleMouseEvent(movieId, x,y, icase, buttonType, scrollDelta);
		}
        ...
        SF_UNITY_EXPORT void SF_CALLING_CONV SF_SetDefaultFontLibName(const char* fontLib)
        {
			Lock::Locker locker(&SFUnityLock);
			if (pManager)
			{
                pManager->SetDefaultFontLibName(fontLib);
			}
        }
